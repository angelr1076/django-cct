document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cancel submit
    document.querySelector('.cancel').addEventListener('submit', cancelAction);
});


// Set timer on messages
setTimeout(function() {
...
const updateCrime = crime_id => {
    ...
    let confMessage = document.querySelector(`#confirmation-${crime_id}`);
    ...
    .then(data => {
        ...
        confMessage.innerHTML = 'Description updated';
        
        setTimeout(function() {
            $(`#confirmation-${crime_id}`).fadeOut('slow');
        }, 3000);
    })
    .catch(error => {
...
const updateSuspect = suspect_id => {
    ...
    let confMessage = document.querySelector(`#confirmation-${suspect_id}`);
    ...
    .then(data => {
        ...
        setTimeout(function() {
            $(`#confirmation-${suspect_id}`).fadeOut('slow');
        }, 3000);
    })
    .catch(error => {
        console.log('Error', error);
